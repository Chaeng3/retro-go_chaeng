...
DEFAULT_APPS = os.getenv("RG_TOOL_APPS", "launcher retro-core prboom-go gwenesis fmsx txt-reader")
...
def build_image(apps, output_file, img_type="odroid", fatsize=0, target="unknown", version="unknown"):
    ...
    firmware_name = globals().get("FIRMWARE_NAME", PROJECT_NAME)
    args = [MKFW_PY, "--type", img_type, "--name", firmware_name, "--icon", PROJECT_ICON, "--version", PROJECT_VER]
...
def build_app(app, device_type, with_profiling=False, no_networking=False, is_release=False):
    ...
    with open("partitions.csv", "w") as f:
        ...
        f.write("dummy, app, ota_0, 65536, 8388608\n")
